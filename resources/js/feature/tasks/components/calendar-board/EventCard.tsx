...
export default function EventCard({
  ...
  const OnClick = (e: React.MouseEvent<HTMLDivElement>) => {
    e.stopPropagation();
    router.visit(`/workspaces/${workspaceId}/tasks/${id}`);
  };
  ...
  return (
    <div className="px-2">
      <div
        onClick={OnClick}
        className={cn(
          "p-1.5 text-xs bg-neutral-900 text-primary border rounded-md border-l-4 flex flex-col gap-y-1.5 cursor-pointer hover:opacity-75 transition",
      ...
      >
        ...
        <div className="flex flex-col gap-x-2">
          <Badge className="capitalize" variant={status as 'todo' | 'in-progress' | 'in-review' | 'done' | 'backlog'}>
