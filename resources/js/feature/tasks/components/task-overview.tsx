      ...
      <div className="bg-muted rounded-lg p-4">
        ...
        <div className="flex flex-col gap-y-4">
          <OverviewProperty label="Assignee">
            <MemberAvatar name={(task.assignee as { name: string })?.name || 'Unassigned'} className="size-6" />

            <p className="text-sm font-medium">{(task.assignee as { name: string })?.name || 'Unassigned'}</p>
          </OverviewProperty>
          ...
          <OverviewProperty label="Status">
            <Badge variant={task.status as 'todo' | 'in-progress' | 'in-review' | 'done' | 'backlog'}>
