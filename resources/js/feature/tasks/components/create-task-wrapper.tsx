...
export default function CreateTaskWrapper({
  ...
  const projectOption = project?.data?.documents?.map((project: { id: number | string; name: string; imageUrl?: string; image_url_full?: string }) => ({
    id: String(project.id),
    name: project.name,
    imageUrl: project.image_url_full || project.imageUrl || '',
  })) || [];

  const memberOption = member?.data?.documents?.map((member: { id: number | string; name: string }) => ({
    id: String(member.id),
    name: member.name,
  })) || [];
