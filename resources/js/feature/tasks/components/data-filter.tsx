...
export default function DataFilter({ hideProjectFilter }: DataFilterProps) {
  const [{ assigneeId, dueDate, projectId, status }, {
    setAssigneeId,
    setDueDate,
    setProjectId,
    setStatus,
  ...
  const projectOption = project?.data?.documents.map((project: { $id: string; name: string; imageUrl?: string; image_url_full?: string }) => ({
    id: project.$id,
    name: project.name,
    imageUrl: project.image_url_full || project.imageUrl,
  }));

  const memberOption = member?.data?.documents.map((member: { $id: string; name: string }) => ({
    id: member.$id,
    name: member.name,
  }));
  const handleStatusChange = (value: string) => {
    setStatus(value === "all" ? null : (value as TasksStatus));
  };
  ...
  return (
    <div className="flex flex-col lg:flex-row lg:items-center gap-2">
      ...
      >
        ...
        <SelectContent>
          {memberOption?.map((member: { id: string; name: string }) => (
      ...
      {!hideProjectFilter && (
        ...
        >
          ...
          <SelectContent>
            {projectOption?.map((project: { id: string; name: string; imageUrl?: string }) => (
